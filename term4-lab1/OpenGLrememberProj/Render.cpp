...
//
//	// Вычисляем радиус (половину длины диаметра)
//	float radius = sqrt(pow((end.x - start.x), 2) + pow((end.y - start.y), 2)) / 2.0f;
//
//	// Определяем углы начала и конца дуги (половины окружности)
...
//
//	// Рисуем дугу окружности (половину окружности)
...
Vertex find_normal(Vertex start, Vertex end, double distance = 1) {
	...
	double nx = -dv.y / line.length;  // Поворачиваем на 90 градусов
