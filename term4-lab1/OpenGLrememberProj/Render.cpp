...
// std::vector<Vertex> make_circle(Vertex center, double radius, double
// start_angle, double end_angle) { 	std::vector<Vertex> circle;
//
//	int count = 100;
...
struct Figure {
  ...
  Figure(double height) {
    vs = {
        {10, 1, height}, // green
        {6, 5, height},   {8, 9, height},
        {2, 11, height},  {5, 16, height}, // blue
        {13, 13, height},                  // blue
        {10, 9, height},  {15, 5, height}  // green
    };
    size_t rows = vs.size();
    ...
    green_circle = new Circle(green_circle_center, green_circle_radius,
    ...
    double blue_circle_end_angle =
        get_angle(vs[5], blue_circle_center) + 2 * pi;
    blue_circle = new Circle(blue_circle_center, blue_circle_radius,
