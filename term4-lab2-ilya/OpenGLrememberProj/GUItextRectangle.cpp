...
public:
  ...
  GuiTextRectanglePrivate() { _tmp = nullptr; }
};
...
void GuiTextRectangle::setSize(int width, int height) {
  ...
  _d->bitmap = CreateDIBSection(0, (BITMAPINFO *)&binfo, DIB_RGB_COLORS,
                                (void **)&(_d->b), 0, 0);
...
int GuiTextRectangle::getWidth() { return d_func()->h; }

int GuiTextRectangle::getHeight() { return d_func()->w; }
...
void GuiTextRectangle::setText(const char *text, char r, char g, char b) {
  ...
  std::fill<byte *, byte>((byte *)(_d->b), (byte *)(_d->b) + _d->w * _d->h * 3,
                          (byte)255);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _d->w, _d->h, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, _tmp);
  delete _tmp;
}
